...
copy_libraries <- function(framework, highlighter, widgets){
	copy_dir(
		from = system.file('libraries', 'frameworks', framework, package = 'slidify2'),
		to = file.path('libraries', 'frameworks', framework)
	)
	copy_dir(
		from = system.file('libraries', 'highlighters', highlighter, package = 'slidify2'),
		to = file.path('libraries', 'highlighters', highlighter)
	)
	for (widget in widgets){
		copy_dir(
			from = system.file('libraries', 'widgets', widget, package = 'slidify2'),
